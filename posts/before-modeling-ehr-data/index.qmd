...
**Note: All examples in this post use synthetic toy data created for educational purposes. They do not contain real patient records, real patient identifiers, institutional data, or results from any specific health system. The examples are informed by literature, but they do not represent any actual patient or clinical case.**
...
This is easy to overlook. At first glance, the data may look fine: same patient, same date, same diagnosis, multiple rows. It is tempting to just remove duplicates with `distinct()` or `unique()` and then move on. But in EHR data, deduplication is not just automated coding step. It is a clinical and analytic decision.
...
One common situation I see is multiple diagnoses codes for the same patient on the same date. At first glance, this looks like a duplicate problem. Sometimes, it is. But often, it is more complicated than that. 
...
If we count each diagnosis code as a separate patient or event, we may overestimate the prevalence of sleep disorders. But, if we collapse all sleep-related codes too early, we many lose clinically useful information about specific conditions (e.g., subtypes).
...
**Tip**: Once define structure, collapse for the analysis, but keep the information somewhere. 
...
This create repeated rows for *AA001*. The insomnia-zolpidem pair is now linked not only to zolpidem, but also to atorvastatin and metformin. THe problem is not that *AA001* has multiple medications. The problem is that the merge attached unrelated medications to the wrong clinical episode. 
...
This can happen during data extraction, SQL joins, repeated pulls, or file appending. This is the type of duplicate where `distinct()` may acutally be appropriate, but only after confirming that the repeated rows are truly identical. 
